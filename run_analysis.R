...
data_with_means <- ddply(dataset1, .(id, activity), .fun=function(x){ colMeans(x[,-c(1:2)]) })

# Adds "_mean" to colnames
colnames(data_with_means)[-c(1:2)] <- paste(colnames(data_with_means)[-c(1:2)], "_mean", sep="")

# Save tidy data_with_means into results folder
saveResult(data_with_means,"data_with_means")